...
(function (api) {
	...
	api.sectionConstructor['creative-portfolio-lite-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.creative-portfolio-lite-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											type: 'GET',
											data: {action: 'creative_portfolio_lite_customizer_notify_dismiss_action', id: id, todo: action},
											dataType: 'html',
											url: creativeportfolioliteCustomizercompanionObject.ajaxurl,
											beforeSend: function () {
												jQuery( '#' + id ).parent().append( '<div id="temp_load" style="text-align:center"><img src="' + creativeportfolioliteCustomizercompanionObject.base_path + '/images/spinner-2x.gif" /></div>' );
											},
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.creative-portfolio-lite-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.creative-portfolio-lite-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.creative-portfolio-lite-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.creative-portfolio-lite-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.creative-portfolio-lite-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
										...
										function activatePlugin(url) {
										    ...
										    jQuery.ajax({
										        beforeSend: function() {
										            jQuery('.activate-now').replaceWith('<a class="button updating-message">' + creativeportfolioliteCustomizercompanionObject.activating_string + '...</a>');
										        },
										        async: true,
										        type: 'GET',
										        url: url,
 ...
 *
 * @package creative_portfolio_lite
 */
...
jQuery( document ).ready(
	function ($) {
		...
		$( 'body' ).on(
			'click', ' .creative-portfolio-lite-install-plugin ', function () {
